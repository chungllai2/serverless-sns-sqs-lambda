...
var ServerlessSnsSqsLambda = /** @class */ (function () {
    ...
    ServerlessSnsSqsLambda.prototype.validateConfig = function (funcName, stage, config) {
        ...
        return __assign(__assign({}, config), { name: config.name, funcName: funcNamePascalCase, prefix: config.prefix || this.serviceName + "-" + stage + "-" + funcNamePascalCase, batchSize: parseIntOr(config.batchSize, 10), maxRetryCount: parseIntOr(config.maxRetryCount, 5), kmsMasterKeyId: config.kmsMasterKeyId, kmsDataKeyReusePeriodSeconds: config.kmsDataKeyReusePeriodSeconds, fifoQueue: config.fifoQueue, fifoThroughputLimit: config.fifoThroughputLimit, deduplicationScope: config.deduplicationScope, deadLetterMessageRetentionPeriodSeconds: config.deadLetterMessageRetentionPeriodSeconds, enabled: config.enabled, visibilityTimeout: config.visibilityTimeout, rawMessageDelivery: config.rawMessageDelivery !== undefined
    ...
    ServerlessSnsSqsLambda.prototype.addEventDeadLetterQueue = function (template, _a) {
        var name = _a.name, prefix = _a.prefix, kmsMasterKeyId = _a.kmsMasterKeyId, kmsDataKeyReusePeriodSeconds = _a.kmsDataKeyReusePeriodSeconds, fifoQueue = _a.fifoQueue, fifoThroughputLimit = _a.fifoThroughputLimit, deduplicationScope = _a.deduplicationScope, deadLetterMessageRetentionPeriodSeconds = _a.deadLetterMessageRetentionPeriodSeconds, deadLetterQueueOverride = _a.deadLetterQueueOverride;
        template.Resources[name + "DeadLetterQueue"] = {
            Type: "AWS::SQS::Queue",
            Properties: __assign(__assign(__assign(__assign(__assign(__assign(__assign({ QueueName: "" + prefix + name + "DeadLetterQueue" }, (kmsMasterKeyId !== undefined
                ? {
    ...
    ServerlessSnsSqsLambda.prototype.addEventQueue = function (template, _a) {
        var name = _a.name, prefix = _a.prefix, maxRetryCount = _a.maxRetryCount, kmsMasterKeyId = _a.kmsMasterKeyId, kmsDataKeyReusePeriodSeconds = _a.kmsDataKeyReusePeriodSeconds, fifoQueue = _a.fifoQueue, fifoThroughputLimit = _a.fifoThroughputLimit, deduplicationScope = _a.deduplicationScope, visibilityTimeout = _a.visibilityTimeout, mainQueueOverride = _a.mainQueueOverride;
        template.Resources[name + "Queue"] = {
            Type: "AWS::SQS::Queue",
            Properties: __assign(__assign(__assign(__assign(__assign(__assign(__assign({ QueueName: "" + prefix + name + "Queue", RedrivePolicy: {
                    deadLetterTargetArn: {
